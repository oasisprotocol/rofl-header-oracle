.PHONY: help install install-dev sync format lint lint-fix check fix test test-watch test-cov clean dev-setup pre-commit ci lint-markdown fix-markdown
.DEFAULT_GOAL := help
...
test-cov: ## Run tests with coverage
	...
	uv run pytest --cov=rofl_oracle --cov-report=term-missing
...
dev-setup: install-dev ## Set up development environment
	...
	@echo "  - Run the application via Docker Compose: '$(GREEN)docker compose -f compose.local.yaml up --build$(NC)'"
...
lint-markdown: ## Lint Markdown files using markdownlint (Docker)
	@echo "$(BLUE)Linting Markdown (docs/) with markdownlint...$(NC)"
...
fix-markdown: ## Auto-fix Markdown issues using markdownlint (Docker)
	@echo "$(BLUE)Fixing Markdown (repo root) with markdownlint --fix...$(NC)"
	docker run --rm -v "$$PWD:/work" -w /work ghcr.io/igorshubovych/markdownlint-cli:latest \
	  markdownlint -c .markdownlint.yml --fix .
